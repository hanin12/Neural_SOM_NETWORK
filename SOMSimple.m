...
ncols = ndim;
nfeature=nfeatures;
som = rand(nrows,ncols,nfeature);

if showMode >= 1
    fig = figure;
    displaySOMmap(fig, 1, 'Randomly initialized SOM', som, nfeature);
end

% Generate random training data
trainingData = rand(ntrainingvectors,nfeature);
...
for t = 1:nepochs    
    ...
    for ntraining = 1:ntrainingvectors
        ...
        dist = getEuclideanDistance(trainingVector, som, nrows, ncols, nfeature);
        ...
        T = reshape(repmat(trainingVector,sz(1)*sz(2),1),sz(1),sz(2),nfeature);                   
        G = repmat(g(fromrow:torow,fromcol:tocol),[1 1 nfeature]);
        ...
        if showMode == 2
            displaySOMmap(fig, 2, ['Epoch: ',num2str(t),'/',num2str(nepochs),', Training Vector: ',num2str(ntraining),'/',num2str(ntrainingvectors)], som, nfeature);
        end        
    end
end

if showMode == 1
    displaySOMmap(fig, 2, 'Trained SOM', som, nfeature);
end
