...
sigma_initial=0.05;
alpha_initial=0.9;
number_of_pixels=3328;%We'll work on full of the dataset.
...
for t=1:T
  alpha=alpha_initial*exp((1-t)/(T/log(alpha_initial)));
    sigma=sigma_initial*exp((1-t)/(T/log(sigma_initial)));
    max_neighbour_radius=round(neighbour_radius_initial*exp((1-t)/(T/log(neighbour_radius_initial))));
       ...
       for neighbour_radius=1:1:max_neighbour_radius
            ...
            if (jj1>=1 ) % to stay in the matrix
               betar=(( R(m,c)-R(jj1,jj2))/(((m-jj1).^2+(c-jj2).^2)*T-1));
                ...
                R(jj1,jj2)=R(jj1,jj2) + alpha * e_factor * (X1(n)-R(m,c))+betar*(R(m,c)-R(jj1,jj2));
                G(jj1,jj2)=G(jj1,jj2) + alpha * e_factor * (X2(n)-G(m,c))+betag*(G(m,c)-G(jj1,jj2));
                B(jj1,jj2)=B(jj1,jj2) + alpha * e_factor * (X3(n)-B(m,c))+betab*(B(m,c)-B(jj1,jj2));
            end
            jj1=m + neighbour_radius;
            jj2=c;
            if (jj1<=52) % to stay in the matrix
            ...
            jj1=m;
            jj2=c + neighbour_radius;
            if (jj2<=64) % to stay in the matrix
...
%AL=csvwrite('balons.txt', reshape(C, size(C,1),[]));
        %image(C)
%colormap(map)
%axis equal
      ...
      %}
       for i=1:52
            for j=1:64
                rgb=A2(i,j,:);
         A2(i,j:64,1) = RED(i,j);
A2(i,j:64,2) = GREEN(i,j);
A2(i,j:64,3) = BLUE(i,j);
            end
       end
       imshow(A2);
       imwrite(A2, 'NewImageName.jpg','Quality', 100);
