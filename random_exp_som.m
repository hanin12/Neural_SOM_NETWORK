...
plot(0.008,1.05,'-s','MarkerSize',100,...
...
plot(0.005,1.3,'-s','MarkerSize',100,...
    'MarkerFaceColor',[I(1,13) I(2,13) I(3,13)]);

plot(0.006,1.3,'-s','MarkerSize',100,...
    'MarkerFaceColor',[I(1,14) I(2,14) I(3,14)]);

plot(0.007,1.3,'-s','MarkerSize',100,...
    'MarkerFaceColor',[I(1,15) I(2,15) I(3,15)]);
plot(0.008,1.3,'-s','MarkerSize',100,...
...
%        e_factor = exp(-((m-m).^2+(c-c).^2)/2*sigma);
       ...
       for neighbour_radius=1:1:max_neighbour_radius
            ...
            jj1=m + neighbour_radius;
            jj2=c;
            if (jj1<=4) % to stay in the matrix
                e_factor = exp(-((m-jj1).^2+(c-jj2).^2)/2*sigma^2);
                  R(jj1,jj2)=R(jj1,jj2) + alpha * e_factor * (I(1,n)-R(jj1,jj2));
                G(jj1,jj2)=G(jj1,jj2) + alpha * e_factor * (I(2,n)-G(jj1,jj2));
                B(jj1,jj2)=B(jj1,jj2) + alpha * e_factor * (I(3,n)-B(jj1,jj2));
            ...
            jj1=m;
            jj2=c + neighbour_radius;
            if (jj2<=4) % to stay in the matrix
                e_factor = exp(-((m-jj1).^2+(c-jj2).^2)/2*sigma^2);
                 R(jj1,jj2)=R(jj1,jj2) + alpha * e_factor * (I(1,n)-R(jj1,jj2));
                G(jj1,jj2)=G(jj1,jj2) + alpha * e_factor * (I(2,n)-G(jj1,jj2));
                B(jj1,jj2)=B(jj1,jj2) + alpha * e_factor * (I(3,n)-B(jj1,jj2));
