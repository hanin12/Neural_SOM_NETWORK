...
for t=1:T
  alpha=alpha_initial*exp(q*(1-t)/(T/log(alpha_initial)));
    sigma=sigma_initial*exp(q*(1-t)/(T/log(sigma_initial)));
    max_neighbour_radius=round(neighbour_radius_initial*exp(q*(1-t)/(T/log(neighbour_radius_initial))));
     ...
     %   e_factor = exp(-((m-m).^2+(c-c).^2)/2*sigma^2);
       ...
       for neighbour_radius=1:1:max_neighbour_radius
            ...
            if (jj1>=1) % to stay in the matrix
               ...
               % e_factor = exp(-(sqrt((m-jj1).^2+(c-jj2).^2))/2*sigma^2);
...
A2(i,j:64,3) = BLUE(i,j);
       ...
       imshow(A2);
        imwrite(A2, 'NewImageNamuer2.jpg','Quality', 100);
